...
class CameraProcess(object):
    ...
    def _save_video(self, delete=False):
        ...
        if (not delete) and (len(self.write_frame_ts) > 0):
            ...
            self.rename_Files(self.write_frame_ts[0],self.write_frame_ts[-1],self.filename) ####
            ret = True
        else:
    ...
    #### Entire funtion
    def rename_Files(self, startTime, endTime, filepath):

            #Get Local Time zone, convert epoch time to date time, then format it as desired.
            timezone = pytz.timezone('Australia/Adelaide')
            self.startTimeStamp = datetime.fromtimestamp(startTime, tz=timezone)
            self.endTimeStamp = datetime.fromtimestamp(endTime, tz=timezone)
            self.startTimeStamp = self.startTimeStamp.strftime("%Y%m%d_%H%M%S.%f")
            self.endTimeStamp = self.endTimeStamp.strftime("%Y%m%d_%H%M%S.%f")

            #Get basename from filepath (remove the filepath and existing timestamp)
            basepath = filepath
            basename = basepath.split('/')
            basename = basename[-1]
            basename = basename.split('_')
            del basename[len(basename)-2]
            basename = '_'.join(basename)

            #Get basepath from filepath (remove the filename from filepath)
            basepath = basepath.split('/')
            del basepath[-1]
            basepath = '/'.join(basepath)

            #### Rename saved video to include start timestamp data
            currentpath = os.path.normpath(f"{filepath}_VIDEO.avi")   
            newpath = os.path.normpath(f"{basepath}/{basename}_{self.startTimeStamp}_{self.endTimeStamp}_VIDEO.avi") 
            if os.path.isfile(currentpath):
                os.rename(currentpath,newpath)

            #### Rename numpy file to include start timestamp data
            currentpath = os.path.normpath(f"{filepath}_TS.npy")   
            newpath = os.path.normpath(f"{basepath}/{basename}_{self.startTimeStamp}_{self.endTimeStamp}_TS.npy") 
            if os.path.isfile(currentpath):
                os.rename(currentpath,newpath)

            #### Rename hdf5 file to include start timestamp data (hdf5 file has not been created yet so it can't be renamed)
            #currentpath = os.path.normpath(f"{filepath}_DLC.hdf5")   
            #newpath = os.path.normpath(f"{basepath}/{basename}_{self.startTimeStamp}_{self.endTimeStamp}_DLC.hdf5") 
            #if os.path.isfile(currentpath):
            #    os.rename(currentpath,newpath)
